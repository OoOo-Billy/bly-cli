    ...
    exec(`npm view ${packageName} version`, function (err, stdout, _stderr) {
      ...
      if (packageName === name) {
        ...
        if (diffType === "major" || diffType === "minor") {
          echo(`\n ✨  New ${chalk.yellow(diffType)} version of bly-cli available! ${chalk.red(local)} → ${chalk.green(stdout)}`);
          echo(" 📖 ", chalk.yellow("See:"), chalk.cyan("https://github.com/OoOo-Billy/bly-cli"));
