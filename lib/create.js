...
async function create(options) {
  ...
  // TODO Let user choice which package manager he wants.
  const packageManager = /**preset.packageManager ||*/ hasYarn()
    ? 'yarn'
    : 'npm'
  ...
  async function creatingFilesAndTemplate() {
    ...
    if (useVue) {
      ...
      for (const dep in packageJson.devDependencies) {
        ...
        packageJson.devDependencies[dep] = /^@vue/.test(dep)
          ? `~${latest}`
          : 'latest'
      }
    ...
    if (useReact) {
      ...
      for (const dep in packageJson.dependencies) {
        packageJson.dependencies[dep] = /react|react-dom/.test(dep)
          ? `^${latest}`
          : 'latest'
      }
    }
    ...
    // TODO should add test or debug mode.
    // if (!DEBUG) {
    await pm.install()
    // }
    ...
    if (!fs.existsSync(path.join(targetDir, './README.md'))) {
      ...
      await writeFileTree(targetDir, {
        'README.md':
          '# This is a test file' /**generateReadme(generator.pkg, packageManager)*/,
      })
    }

    //TODO developing here.
    setTimeout(() => {
      ...
      if (true) {
        echo(
          `\n👉  Get started by typing:\n\n` +
            ...
            chalk.cyan(
              ` ${chalk.gray('$')} ${
                packageManager === 'yarn' ? 'yarn start' : 'npm start'
              }`
            )
  ...
  function resolvePrompts() {
    ...
    const prompts = [templatePrompt, featurePrompt, ...injectedPrompts]
    return prompts
  }
